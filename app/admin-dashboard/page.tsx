...
const STORAGE_KEY = "admin_notif_state";
...
export default function AdminDashboard() {
  ...
  const fetchCounts = async () => {
    try {
      ...
      const [users, staff, events, jobs] = await Promise.all([
        usersRes.json(), staffRes.json(), eventsRes.json(), jobsRes.json(),
      ]);
  ...
  const stats = [
    { label: "Total Alumni", value: totalUsers, icon: Users, href: "/admin-dashboard/user-manage", gradient: "from-blue-500/20 via-blue-600/10 to-transparent", border: "border-blue-500/30", iconBg: "bg-blue-500/15", iconColor: "text-blue-400", glow: "shadow-blue-500/10" },
    { label: "Staff Accounts", value: totalStaff, icon: Shield, href: "/admin-dashboard/staff-manage", gradient: "from-purple-500/20 via-purple-600/10 to-transparent", border: "border-purple-500/30", iconBg: "bg-purple-500/15", iconColor: "text-purple-400", glow: "shadow-purple-500/10" },
    { label: "Total Events", value: totalEvents, icon: Calendar, href: "/admin-dashboard/view-eventadmin", gradient: "from-emerald-500/20 via-emerald-600/10 to-transparent", border: "border-emerald-500/30", iconBg: "bg-emerald-500/15", iconColor: "text-emerald-400", glow: "shadow-emerald-500/10" },
    { label: "Job Postings", value: totalJobs, icon: Search, href: "/admin-dashboard/view-jobadmin", gradient: "from-yellow-500/20 via-yellow-600/10 to-transparent", border: "border-yellow-500/30", iconBg: "bg-yellow-500/15", iconColor: "text-yellow-400", glow: "shadow-yellow-500/10" },
  ];

  const quickLinks = [
    { href: "/admin-dashboard/user-manage", label: "Alumni Management", icon: Users, desc: "View and manage alumni records", accent: "blue" },
                    ...
                    {visibleUsers.length > 0 && (
                      <div>
                        ...
                        {visibleUsers.map((u) => {
                          ...
                          return (
                            <Link key={u.id} href="/admin-dashboard/user-manage"
                    ...
                    {visibleEventRegs.length > 0 && (
                      <div>
                        ...
                        {visibleEventRegs.map((r) => {
                          ...
                          const isRead = readIds.has(key);
                          return (
                            <Link key={r.id} href="/admin-dashboard/view-eventadmin"
                              onClick={() => { markRead(key); setShowNotif(false); }}
                              className={`flex items-start gap-3 px-4 py-3 hover:bg-blue-900/30 transition-colors border-b border-blue-900/30 group/item ${
                                isRead ? "opacity-50" : ""
                              }`}>
                    ...
                    {visibleReports.length > 0 && (
                      <div>
                        ...
                        {visibleReports.map((r) => {
                          ...
                          const isRead = readIds.has(key);
                          return (
                            <Link key={r.id} href="/admin-dashboard/adminreports-view"
                              onClick={() => { markRead(key); setShowNotif(false); }}
                              className={`flex items-start gap-3 px-4 py-3 hover:bg-blue-900/30 transition-colors border-b border-blue-900/30 group/item ${
                                isRead ? "opacity-50" : ""
                              }`}>
        ...
        <main className="p-4 lg:p-8">
          ...
          <div className="relative mb-8 p-6 rounded-2xl bg-gradient-to-r from-yellow-500/10 via-blue-900/20 to-transparent border border-yellow-500/15 overflow-hidden">
            ...
            <div className="relative flex items-center justify-between">
              ...
              <Activity size={48} className="text-yellow-400/20" />
            </div>
          </div>
