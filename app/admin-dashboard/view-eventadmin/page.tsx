      ...
      <main className="lg:ml-64 flex-1 p-4 lg:p-8 pt-20 lg:pt-8">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-400 ml-12">Create and manage alumni events</p>
          </div>
          <button
            onClick={() => router.push("/admin-dashboard/add-admin")}
        ...
        <div className="relative mb-6">
          ...
          <input
            type="text"
            placeholder="Search by Title, Location, or Date (YYYY-MM-DD)..."
            value={searchTerm}
              ...
              <div key={event.id} className="bg-white/3 border border-white/8 rounded-2xl overflow-hidden hover:border-yellow-500/30 transition-all group">
                ...
                <div className="p-5">
                  ...
                  <div className="space-y-1.5 mb-4">
                    ...
                    {event.batch_category && (
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <span className="px-2 py-0.5 bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 rounded-full">Batch: {event.batch_category}</span>
                      </div>
                    )}
                    {event.course_category && (
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <BookOpen size={11} className="text-purple-400/60" />
                        <span className="px-2 py-0.5 bg-purple-500/10 border border-purple-500/20 text-purple-300 rounded-full">{event.course_category}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 pt-3 border-t border-white/8">
                    ...
                    <Link href={`/admin-dashboard/admin-eventlist/${event.id}`}
