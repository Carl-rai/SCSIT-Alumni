...
import { Users, Calendar, Search, Shield, FileText, LogOut, LayoutDashboard, Menu, X, ShieldAlert } from "lucide-react";
...
function SidebarContent({ pathname, onClose, onRequestLogout }: SidebarContentProps) {
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-5 border-b border-blue-800/50">
        <div className="flex items-center gap-3">
          <Image src="/nah.png" alt="Logo" width={36} height={40} className="h-9 w-8 object-contain" />
          <div>
      ...
      <div className="px-3 py-4 border-t border-blue-800/50">
        <button onClick={onRequestLogout}
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 bg-blue-950 border-b border-blue-800/50 shadow-lg">
        <div className="flex items-center gap-3">
          <Image src="/nah.png" alt="Logo" width={32} height={36} className="h-8 w-7 object-contain" />
          <div>
      ...
      <aside className={`lg:hidden fixed top-0 left-0 h-full w-72 bg-blue-950 border-r border-blue-800/50 z-50 shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <SidebarContent pathname={pathname} onClose={() => setOpen(false)} onRequestLogout={() => setShowLogoutConfirm(true)} />
      </aside>
