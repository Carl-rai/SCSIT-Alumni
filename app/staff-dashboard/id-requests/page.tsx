...
export default function StaffIDRequestsPage() {
  ...
  useEffect(() => {
    ...
    if (role !== "staff" && role !== "admin") { router.push("/"); return; }
    fetchRequests();
  ...
  const handleExport = () => {
    ...
    const url = `/api/id-requests/export/?status=${statusFilter}`;
    const a = document.createElement("a");
    a.href = url;
    a.setAttribute("download", `id_requests_${statusFilter}.csv`);
    ...
    fetch(url, { headers: { Authorization: `Bearer ${getToken()}` } })
      ...
      .then((blob) => {
        ...
        // refresh list after export marks them as exported
      ...
      <main className="lg:ml-64 flex-1 p-4 lg:p-8 pt-20 lg:pt-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
              <IdCard size={22} className="text-yellow-400" />
            </div>
            <h1 className="text-3xl font-bold text-white">Alumni ID <span className="text-yellow-400">Requests</span></h1>
          </div>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <select
            ...
            className="px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-yellow-500/50"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-yellow-500 hover:bg-yellow-400 disabled:opacity-50 text-blue-950 font-bold rounded-xl text-sm transition-all"
          >
                ...
                <thead>
                  <tr className="border-b border-white/8 bg-white/5">
                    {["Alumni ID", "Name", "Email", "Course", "Year", "Gender", "Note", "Status", "Requested"].concat(showActions ? ["Actions"] : []).map((h) => (
                      <th key={h} className="px-4 py-4 text-left text-xs font-semibold text-yellow-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                      ...
                      {showActions && (
                        <td className="px-4 py-3">
                          <select
                            ...
                            className="min-w-[120px] rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-white focus:outline-none focus:border-yellow-500/50 disabled:opacity-50"
                          >
