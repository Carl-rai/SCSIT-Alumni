    ...
    <div className="flex min-h-screen bg-[#020d1f] text-gray-100">
      ...
      <main className="lg:ml-64 flex-1 p-4 lg:p-8 pt-20 lg:pt-8">
        <div className="mb-8">
          ...
          {eventTitle && <p className="text-gray-400 ml-12">Event: <span className="text-white font-medium">{eventTitle}</span></p>}
        </div>

        <div className="relative mb-6">
          <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
          <input type="text" placeholder="Search by Alumni ID, Name, or Email..."
                ...
                <tbody>
                  {filtered.map((r, i) => (
                    <tr key={r.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm font-mono">{r.user_details?.alumni_id || <span className="text-red-400 italic text-xs">missing</span>}</td>
