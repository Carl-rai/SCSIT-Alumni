...
export default function StaffAlumniRecordsPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/alumni-students/${id}/`), { method: "DELETE" });
      if (res.ok) { alert("Deleted!"); fetchStudents(); }
    ...
    <div className="flex min-h-screen bg-[#020d1f] text-gray-100">
      ...
      <main className="lg:ml-64 flex-1 p-4 lg:p-8 pt-20 lg:pt-8">
        <div className="mb-8 flex items-start justify-between">
          ...
          <Link href="/staff-dashboard/staff-alumnirecordsadd"
        ...
        <div className="relative mb-6">
          ...
          <input type="text" placeholder="Search by Alumni ID, Name, Year, or Course..."
                    ...
                    <tr key={s.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-1.5">
                          <button onClick={() => router.push(`/staff-dashboard/staff-editalumni/${s.id}`)}
